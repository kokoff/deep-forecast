...
file.remove(list.files(paste(WORK_DIR, '/', 'acf', '/', sep = ''), full.names = TRUE))
file.remove(list.files(paste(WORK_DIR, '/', 'pacf', '/', sep = ''), full.names = TRUE))
file.remove(list.files(paste(WORK_DIR, '/', 'stationary_tests', '/', sep = ''), full.names = TRUE))
...
for (i in names(data)){
  for (j in names(data[[i]])){
    path <- paste(WORK_DIR,'/', 'plots', '/', i, '_', j, ".png", sep="")
    png(filename=path)
    series <- data[[i]][[j]] 
    plot(series)
    dev.off()
...
for (i in names(data)){
  for (j in names(data[[i]])){
    path <- paste(WORK_DIR,'/', 'acf', '/', i, '_', j, ".png", sep="")
    png(filename=path)
    series <- data[[i]][[j]] 
    acf(series,na.action = na.pass)
    dev.off()
  }
}
...
for (i in names(data)){
  for (j in names(data[[i]])){
    path <- paste(WORK_DIR,'/', 'pacf', '/', i, '_', j, ".png", sep="")
    png(filename=path)
    series <- data[[i]][[j]] 
    pacf(series, na.action = na.pass)
    dev.off()
  }
}
...
filepath <- paste(WORK_DIR, 'stationary_tests', 'test_results.txt', sep = '/')
sink(filepath)

for (i in names(data)){
  for (j in names(data[[i]])){
    filename <- paste(i, '_',  j, '.csv', sep = '')
