...
for (country in names(data)) {
    for (name in names(data[[country]])) {
        ...
        p = autoplot(series, series = 'observed', ylab = y_lab) +
            autolayer(prediction.train, series = 'prediction train') +
            autolayer(prediction.test, series = 'prediction test') +
            ggtitle(paste('One step prediction from ', model, sep = '')) +
        ...
        p = autoplot(fcast, ylab = y_lab) +
            autolayer(series, series = 'observed') +
            autolayer(fcast$mean, series = 'forecast') +
            ggtitle(paste('Forecasts from ', model, sep = '')) +
            theme(legend.position = "bottom") +
            theme(text = element_text(size = 16))
        print(p)
